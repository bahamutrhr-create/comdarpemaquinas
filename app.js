...
function openModal(machine) {
  ...
  modal.innerHTML = `
    ...
    <button class="btn-primary" id="ticketBtn" style="background:var(--amber);color:#1a0f00;">Ticket de manutenção</button>
    <button class="btn-secondary" id="historyBtn">Histórico de manutenção</button>
  ...
  document.getElementById("ticketBtn").onclick = () => openTicketForm(machine);
  document.getElementById("historyBtn").onclick = () => openTicketHistory(machine);
...
function openTicketForm(machine) {
  const overlay = document.getElementById("modalOverlay");
  const modal = document.getElementById("modal");

  modal.innerHTML = `
    <button class="modal-close" id="modalClose">&times;</button>
    <h2>Ticket de manutenção</h2>
    <div class="modal-sub">${machine.name} · TAG ${machine.tag}</div>

    <div class="modal-field"><span class="label">Modelo</span><span class="value">${machine.name || "-"}</span></div>
    <div class="modal-field"><span class="label">TAG</span><span class="value">${machine.tag}</span></div>

    <div style="display:flex;flex-direction:column;gap:12px;margin-top:12px;">
      <input class="form-input" id="tHorimetro" type="number" step="0.1" placeholder="Horímetro atual" value="${machine.horimetro || 0}" />
      <select class="form-input" id="tTipo">
        <option value="Corretiva">Manutenção Corretiva</option>
        <option value="Preventiva">Manutenção Preventiva</option>
      </select>
      <textarea class="form-input" id="tDescricao" rows="4" placeholder="Descreva o problema ou serviço necessário..." style="resize:vertical;font-family:inherit;"></textarea>
      <input class="form-input" id="tResponsavel" placeholder="Seu nome (opcional)" />
      <div style="font-size:12.5px;color:var(--text-faint);">Fotos (pode escolher várias):</div>
      <input type="file" accept="image/*" id="tFotos" multiple />
    </div>

    <button class="btn-primary" id="submitTicketBtn" style="background:var(--amber);color:#1a0f00;">Abrir ticket</button>
    <button class="btn-secondary" id="cancelBtn">Cancelar</button>
    <div id="saveStatus" style="text-align:center;font-size:12.5px;color:var(--text-faint);margin-top:8px;"></div>
  `;
  ...
  document.getElementById("modalClose").onclick = closeModal;
  document.getElementById("cancelBtn").onclick = closeModal;
  document.getElementById("submitTicketBtn").onclick = () => submitTicket(machine);
}

async function submitTicket(machine) {
  const horimetro = parseFloat(document.getElementById("tHorimetro").value) || 0;
  const tipo = document.getElementById("tTipo").value;
  const descricao = document.getElementById("tDescricao").value.trim();
  const responsavel = document.getElementById("tResponsavel").value.trim();
  const files = Array.from(document.getElementById("tFotos").files || []);
  const statusEl = document.getElementById("saveStatus");

  if (!descricao) {
    statusEl.textContent = "Descreva o problema ou serviço necessário.";
    return;
  }
  if (!USING_REAL_BACKEND) {
    statusEl.textContent = "Configure a API_URL em config.js primeiro.";
    return;
  }

  // otimista: fecha e avisa na hora, envia em segundo plano
  closeModal();
  showToast("Abrindo ticket...", "info", 60000);
  ...
  (async () => {
    try {
      ...
      for (const file of files) {
        ...
        photos.push({ base64, mimeType: file.type });
      }
      await createTicket({
      ...
      showToast("Ticket aberto ✓ e-mail enviado", "success");
    } catch (err) {
      showToast("Erro ao abrir o ticket. Tente novamente.", "error", 4000);
    }
  })();
}

function openTicketHistory(machine) {
  const overlay = document.getElementById("modalOverlay");
  const modal = document.getElementById("modal");
  ...
  modal.innerHTML = `
    ...
    <div class="modal-sub">${machine.name} · TAG ${machine.tag}</div>
    <div id="ticketHistoryList" style="margin-top:12px;">Carregando...</div>
    <button class="btn-secondary" id="closeBtn2" style="margin-top:16px;">Fechar</button>
  `;
  overlay.classList.add("open");
  document.getElementById("modalClose").onclick = closeModal;
  document.getElementById("closeBtn2").onclick = closeModal;

  fetchTickets(machine.id)
    .then((tickets) => {
      const container = document.getElementById("ticketHistoryList");
      if (!container) return; // modal já foi fechado
      if (!tickets.length) {
        container.innerHTML = `<div class="empty-state">Nenhum ticket registrado ainda.</div>`;
        return;
      }
      container.innerHTML = tickets
        ...
        <div style="border:1px solid var(--card-border);border-radius:12px;padding:12px;margin-bottom:10px;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
            <span style="font-weight:700;font-size:13.5px;">${t.tipo}</span>
            <span style="font-size:11px;font-weight:700;color:${t.status === "Aberto" ? "var(--amber)" : "var(--green)"};">${t.status}</span>
          </div>
          <div style="font-size:12.5px;color:var(--text-dim);margin-bottom:4px;">${t.dataAbertura} · Horímetro: ${(t.horimetro || 0).toLocaleString("pt-BR")} h</div>
          <div style="font-size:13px;">${t.descricao || ""}</div>
          ${t.responsavel ? `<div style="font-size:11.5px;color:var(--text-faint);margin-top:4px;">Registrado por: ${t.responsavel}</div>` : ""}
          ${
            t.fotos && t.fotos.length
              ? `<div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap;">${t.fotos
                  .map((url) => `<img src="${url}" style="width:52px;height:52px;border-radius:8px;object-fit:cover;" />`)
                  .join("")}</div>`
              : ""
          }
        </div>`
    ...
    .catch(() => {
      const container = document.getElementById("ticketHistoryList");
      if (container) container.innerHTML = `<div class="empty-state">Não foi possível carregar o histórico.</div>`;
    });
}
